...
# --- PAGE CONFIGURATION ---
st.set_page_config(page_title="Army Eval Planner v6.0", page_icon="🦅", layout="wide")
...
# --- MAIN APP ---
def main():
    st.title("🦅 Army Evaluation Planner & Kiosk v6.0")
    st.markdown("Strictly automated IPPS-A date math, Command Profile Ledger, and Text Formatting.")

    tab1, tab2, tab3 = st.tabs(["📅 Dates & Output", "📋 Command Profile Ledger", "📝 IPPS-A Text Counter"])
    ...
    with tab1:
        ...
        st.header("2. Base Rating & Signatures")
        ...
        with col3: rater_sig_str = st.text_input("Rater Sig Date (Opt)", max_chars=8)
        with col4: sr_sig_str = st.text_input("SR Sig Date (Opt)", max_chars=8)
        ...
        if len(from_str) == 8 and len(thru_str) == 8:
            ...
            if len(sr_sig_str) == 8:
                sr_date = parse_date(sr_sig_str)
                if sr_date and sr_date < thru_date:
                    st.warning("⚠️ **SIGNATURE WARNING:** The Senior Rater is signing before the Thru Date. Ensure this meets an authorized exception.")
    ...
    with tab2:
        ...
        # Initialize Default Ledger in specific order if not loaded
        ...
        # Update session state for exporting
    ...
    # ==========================================
    # TAB 3: IPPS-A TEXT FORMATTER
    # ==========================================
    with tab3:
        st.header("📝 IPPS-A Text Formatting & Constraint Check")
        st.markdown("Use this to draft performance narratives and bullets. It actively monitors line breaks and character limits to ensure a clean copy/paste into IPPS-A.")
        
        text_input = st.text_area("Draft Evaluation Text (Paste here)", height=250)
        ...
        if text_input:
            ...
            col_t1.metric("Character Count", char_count)
            col_t2.metric("Line Count (Hard Returns)", line_count)
